...
Figure X provides simple descriptive statistics about off-campus recruiting visits (e.g., visits to local high schools, community colleges, hotel receptions) by the University of Alabama in the 2017 calendar year [REVISE PARAGRAPH IN RELATION TO FIGURE].  Admissions representatives made \Sexpr{num_visits_all_100751} off-campus recruiting visits.  However, only \Sexpr{num_visits_in_100751} of these visits occurred in Alabama.  Further, the University visited only 32\% of Alabama public high schools. These in-state public high school visits were concentrated relatively, affluent, predominantly White communities, largely avoiding high schools in Alabama's ``Black Belt,'' which enroll the largest concentration of African American Students.  However, these in-state recruiting efforts were dwarfed by the \Sexpr{num_visits_out_100751} out-of-state recruiting visits, which spanned metropolitan areas across the U.S. The University made \Sexpr{num_pubHS_out_100751} visits to out-of-state public high schools. These visits focused on schools in affluent communities, with visited schools having a much higher percent of White students than non-visited schools.  Incredibly, the University made \Sexpr{num_privHS_out_100751} visits to out-of-state private high schools, more than double the total number of in-state recruiting visits.
...
\lipsum[4]
% Params: univ_id (which is also the name of the folder inside graphs/ folder, where the images are located), caption, fig reference, top or bottom of page
\addFigureSet{139959}{University of Georgia result set.}{uga}{b}
\lipsum[5]
...
\lipsum[4]
\addFigureSet{215293}{University of Pittsburgh result set.}{upitt}{b}
\lipsum[5]
...
\begingroup
  ...
  \input{tables/event_count.tex}
\endgroup

\begin{landscape}
  \fontsize{8pt}{10pt}\selectfont
    \input{tables/desc_in_pub.tex}
  
  \newpage
  \input{tables/desc_in_cc.tex}
  
  \newpage
  \input{tables/desc_out_pub.tex}
  
  \newpage
  \input{tables/desc_out_priv.tex}
\end{landscape}

\begingroup
  \fontsize{8pt}{10pt}\selectfont
  \input{tables/regression_in.tex}
\endgroup
...
\begingroup
  \fontsize{8pt}{10pt}\selectfont
  \input{tables/regression_out.tex}
\endgroup

\begin{landscape}
  \fontsize{8pt}{7.8pt}\selectfont
  \input{tables/regression_out.tex}
\end{landscape}

\clearpage
\input{tables/event_count_alt.tex}

\clearpage
\input{tables/desc_out.tex}

\clearpage
\input{tables/regression_out_alt.tex}

\clearpage
% Params: graph_type, caption, fig reference
\addSmallMultiples{titled_map}{Map of visits.}{map}

\clearpage
\input{tables/desc_in.tex}

\clearpage
\input{tables/regression_in_alt.tex}
